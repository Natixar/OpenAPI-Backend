...
@router.get('/{scenarioId}/last-id', responses={(200): {'model': int,
    'description': 'OK'}, (400): {'model': GetLastId400Response,
    'description': 'Bad Request'}, (404): {'model': GetLastId400Response,
    'description': 'Not Found'}}, tags=['default'], summary=
    'Get the last-id in the scenario.', response_model_by_alias=True)
async def get_last_id(scenarioId: str=Path(..., description=
    'UUID of a scenario inserted as a path element of an URI.')) ->int:
...
@router.post('/{scenarioId}', responses={(201): {'model': str,
    'description': 'Created'}, (400): {'model': GetLastId400Response,
    'description': 'Bad Request'}, (404): {'model': GetLastId400Response,
    'description': 'Not Found'}}, tags=['default'], summary=
    'Post data to a specific scenario via the Klio ERP connector.',
    response_model_by_alias=True)
async def post_erp_data(scenarioId: str=Path(..., description=
    'UUID of a scenario inserted as a path element of an URI.'),
    post_erp_data_request: PostErpDataRequest=Body(None, description='')
    ) ->str:
    ...
    return await post_erp_data_impl(scenarioId=scenarioId,
        post_erp_data_request=post_erp_data_request)
